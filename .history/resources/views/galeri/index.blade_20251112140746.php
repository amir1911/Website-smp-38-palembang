...
>
    ...
    >
        ...
        <button 
            @click="closeModal()"
            class="absolute top-4 right-4 z-50 bg-white/10 hover:bg-white/20 backdrop-blur-sm text-white p-2 sm:p-3 rounded-full transition-all duration-300 hover:rotate-90 group"
        >
        ...
        <button 
            @click="prev()"
            class="absolute left-2 sm:left-4 z-50 bg-white/10 hover:bg-white/20 backdrop-blur-sm text-white p-2 sm:p-4 rounded-full transition-all duration-300 hover:scale-110"
        ...
        <button 
            @click="next()"
            class="absolute right-2 sm:right-4 z-50 bg-white/10 hover:bg-white/20 backdrop-blur-sm text-white p-2 sm:p-4 rounded-full transition-all duration-300 hover:scale-110"
            x-show="filteredItems.length > 1"
        >
            <svg class="w-6 h-6 sm:w-8 sm:h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="relative w-full max-w-6xl mx-auto">
            ...
            >
                <img 
                    ...
                    class="w-full h-auto max-h-[70vh] sm:max-h-[80vh] object-contain mx-auto"
                    :key="currentIndex"
                >
            </div>

            {{-- Image Info --}}
            <div class="mt-4 sm:mt-6 bg-white/10 backdrop-blur-md rounded-xl p-4 sm:p-6 text-white">
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                    <div class="flex-1">
                        <div class="flex items-center gap-2 mb-2">
                            ...
                            ></span>
                            <span class="text-gray-300 text-sm" x-text="`${currentIndex + 1} / ${filteredItems.length}`"></span>
                        </div>
                        <h3 class="text-xl sm:text-2xl font-bold mb-2" x-text="filteredItems[currentIndex]?.judul"></h3>
                        <p class="text-gray-300 text-sm sm:text-base" x-text="filteredItems[currentIndex]?.deskripsi ?? 'Tidak ada deskripsi'"></p>
                    </div>
                </div>
            </div>
            ...
            <div 
                class="mt-4 flex gap-2 overflow-x-auto pb-2 scrollbar-thin scrollbar-thumb-white/20 scrollbar-track-transparent"
                x-show="filteredItems.length > 1"
            >
                <template x-for="(item, idx) in filteredItems" :key="item.id">
                    <button
                        @click="goTo(idx)"
                        :class="idx === currentIndex ? 'ring-4 ring-blue-500 opacity-100' : 'opacity-50 hover:opacity-100'"
                        class="flex-shrink-0 w-16 h-16 sm:w-20 sm:h-20 rounded-lg overflow-hidden transition-all duration-300"
                    >
                        <img 
                            :src="`/storage/${item.foto}`"
                            :alt="item.judul"
