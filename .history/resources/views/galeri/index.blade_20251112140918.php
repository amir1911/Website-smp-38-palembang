...
>
    ...
    >
        ...
        <button 
            @click="closeModal()"
            class="absolute top-2 right-2 sm:top-4 sm:right-4 z-50 bg-white/10 hover:bg-white/20 backdrop-blur-sm text-white p-2 sm:p-3 rounded-full transition-all duration-300 hover:rotate-90"
        >
        ...
        <button 
            @click="prev()"
            class="hidden md:flex absolute left-4 top-1/2 -translate-y-1/2 z-50 bg-white/10 hover:bg-white/20 backdrop-blur-sm text-white p-3 lg:p-4 rounded-full transition-all duration-300 hover:scale-110 items-center justify-center"
        ...
        <button 
            @click="next()"
            class="hidden md:flex absolute right-4 top-1/2 -translate-y-1/2 z-50 bg-white/10 hover:bg-white/20 backdrop-blur-sm text-white p-3 lg:p-4 rounded-full transition-all duration-300 hover:scale-110 items-center justify-center"
            x-show="filteredItems.length > 1"
        >
            <svg class="w-6 h-6 lg:w-8 lg:h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="h-full flex flex-col">
            ...
            <div class="flex-1 flex items-center justify-center px-2 sm:px-4 pt-14 sm:pt-16 pb-2">
                <div class="relative w-full max-w-7xl">
                    <div 
                        class="relative bg-black rounded-none sm:rounded-xl overflow-hidden shadow-2xl"
                        x-transition:enter="transition ease-out duration-300"
                    ...
                    >
                        <img 
                            ...
                            class="w-full h-auto max-h-[50vh] sm:max-h-[65vh] lg:max-h-[75vh] object-contain mx-auto"
                            :key="currentIndex"
                        >
                    </div>
                </div>
            ...
            {{-- Info & Navigation Section - Fixed Bottom --}}
            <div class="flex-shrink-0 px-2 sm:px-4 pb-2 sm:pb-4">
                <div class="max-w-7xl mx-auto">
                    {{-- Image Info --}}
                    <div class="bg-white/10 backdrop-blur-md rounded-xl sm:rounded-2xl p-3 sm:p-4 lg:p-6 text-white mb-2 sm:mb-3">
                        <div class="flex items-start justify-between gap-2 sm:gap-4">
                            <div class="flex-1 min-w-0">
                                <div class="flex items-center gap-2 mb-1 sm:mb-2 flex-wrap">
                                    <span 
                                        class="bg-blue-500 text-white text-xs font-semibold px-2.5 sm:px-3 py-1 rounded-full whitespace-nowrap"
                                        x-text="filteredItems[currentIndex]?.kategori?.nama ?? 'Umum'"
                                    ></span>
                                    <span class="text-gray-300 text-xs sm:text-sm whitespace-nowrap" x-text="`${currentIndex + 1} / ${filteredItems.length}`"></span>
                                </div>
                                <h3 class="text-base sm:text-lg lg:text-xl font-bold mb-1 truncate" x-text="filteredItems[currentIndex]?.judul"></h3>
                            ...
                            <div class="flex md:hidden gap-1 flex-shrink-0" x-show="filteredItems.length > 1">
                                <button 
                                    @click="prev()"
                                    class="bg-white/10 hover:bg-white/20 backdrop-blur-sm text-white p-2 rounded-lg transition-all"
                                >
                    ...
                    <div 
                        class="flex gap-1.5 sm:gap-2 overflow-x-auto pb-1 sm:pb-2 scrollbar-thin scrollbar-thumb-white/20 scrollbar-track-transparent snap-x snap-mandatory"
                        x-show="filteredItems.length > 1"
                    >
                        <template x-for="(item, idx) in filteredItems" :key="item.id">
                            <button
                                @click="goTo(idx)"
                                :class="idx === currentIndex ? 'ring-2 sm:ring-4 ring-blue-500 opacity-100' : 'opacity-50 hover:opacity-100'"
                                class="flex-shrink-0 w-12 h-12 sm:w-16 sm:h-16 lg:w-20 lg:h-20 rounded-md sm:rounded-lg overflow-hidden transition-all duration-300 snap-center"
                            >
                                <img 
                                    :src="`/storage/${item.foto}`"
                                    :alt="item.judul"
