...
@section('content')
    ...
    <div x-data="galeriCarousel({{ $galeri->toJson() }}, {{ $kategori->toJson() }})" class="w-full px-4 sm:px-6 md:px-12 lg:px-20 py-12 sm:py-16 md:py-24 -mb-20 min-h-screen"
        ...
        <div class="max-w-6xl mx-auto">
            ...
            <div class="text-center mb-8 sm:mb-10 md:mb-12">
                ...
                <!-- Badge GALERI -->
                <div
                    class="inline-block bg-white/20 backdrop-blur-sm px-7 sm:px-8 md:px-10 py-2 sm:py-2.5 rounded-full mb-3 border border-white/30 shadow-md">
                ...
                <!-- Judul Utama -->
                <h2
                    class="text-white font-extrabold tracking-wide mb-2
                ...
                <template x-if="filteredItems.length">
                    <div class="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3 sm:gap-4 md:gap-6">
                        <template x-for="(item, index) in filteredItems" :key="item.id">
                            <div class="group relative overflow-hidden rounded-xl sm:rounded-2xl cursor-pointer transform hover:scale-[1.02] transition-all duration-500 bg-gray-100 shadow-md hover:shadow-xl"
                                ...
                                <div class="relative overflow-hidden aspect-[4/3]">
                                    <img :src="`/storage/${item.foto}`" :alt="item.judul"
                                        ...
                                        loading="lazy">
                                    <div
                                        class="absolute inset-0 bg-gradient-to-t from-black/70 via-black/30 to-transparent opacity-0 group-hover:opacity-100 transition-all duration-300">
                                    </div>

                                    <!-- Category Badge -->
                                    <div class="absolute top-2 right-2 sm:top-3 sm:right-3">
                                        <span
                                            class="text-white text-[10px] sm:text-xs font-bold px-2 sm:px-3 py-1 rounded-full shadow-md"
                                    ...
                                    <!-- Info Overlay -->
                                    <div
                                        class="absolute bottom-0 left-0 right-0 p-3 sm:p-4 transform translate-y-full group-hover:translate-y-0 transition-transform duration-300">
                                        <h3 class="text-white font-bold text-xs sm:text-sm md:text-base mb-1 line-clamp-1"
                                            x-text="item.judul"></h3>
                                        <p class="text-gray-200 text-[10px] sm:text-xs line-clamp-2 hidden sm:block"
        ...
        <div x-show="modalOpen" x-cloak x-transition:enter="transition ease-out duration-300"
            ...
            x-transition:leave-end="opacity-0" @keydown.escape.window="closeModal()" @keydown.arrow-left.window="prev()"
            @keydown.arrow-right.window="next()" class="fixed inset-0 z-[9999] bg-black/95 flex flex-col">
            ...
            <div class="flex-1 flex items-center justify-center relative px-4">
                ...
                <!-- Image -->
                <img :src="`/storage/${filteredItems[currentIndex]?.foto}`" :alt="filteredItems[currentIndex]?.judul"
                    class="max-h-[70vh] max-w-full object-contain rounded-lg" :key="currentIndex">
            ...
            <div class="bg-black/50 px-4 py-4">
                <div class="max-w-4xl mx-auto">
                    <h3 class="text-white font-bold text-sm sm:text-base mb-1" x-text="filteredItems[currentIndex]?.judul">
                    </h3>
                    <p class="text-white/70 text-xs sm:text-sm mb-3"
                    ...
                    <div class="flex gap-2 overflow-x-auto pb-2" x-show="filteredItems.length > 1">
                        <template x-for="(item, idx) in filteredItems" :key="item.id">
                            <button @click="goTo(idx)"
                                :class="idx === currentIndex ? 'ring-2 ring-blue-500 opacity-100' :
