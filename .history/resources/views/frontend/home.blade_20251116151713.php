...
<!-- Minimalist Carousel -->
<!-- Sophisticated Blue Glass Carousel -->
<section class="relative w-full overflow-hidden font-[Poppins] bg-gradient-to-b from-white via-sky-50/30 to-blue-50/50" 
    ...
    <template x-for="(item, index) in {{ $carousels->toJson() }}" :key="index">
        <div 
            ...
            <div class="absolute inset-6 sm:inset-10 md:inset-14 lg:inset-16">
                <!-- Image with sophisticated frame -->
                <div class="relative w-full h-full overflow-hidden">
                    ...
                    <!-- Layered glass overlays -->
                    <div class="absolute inset-0 bg-gradient-to-br from-sky-600/80 via-blue-500/70 to-cyan-600/75"></div>
                    <div class="absolute inset-0 bg-gradient-to-t from-slate-900/60 via-transparent to-transparent"></div>
                    
                    <!-- Frosted glass effect on edges -->
                    <div class="absolute inset-0 backdrop-blur-[0.5px]"></div>
                </div>

                <!-- Top decorative bar -->
                <div class="absolute -top-1 left-0 right-0 flex items-center justify-between px-8 py-4">
                    <div class="flex items-center gap-3">
                        <div class="w-12 h-1 bg-gradient-to-r from-sky-400 to-transparent rounded-full"></div>
                        <svg class="w-5 h-5 text-sky-300/60" fill="currentColor" viewBox="0 0 20 20">
                            <path d="M10.394 2.08a1 1 0 00-.788 0l-7 3a1 1 0 000 1.84L5.25 8.051a.999.999 0 01.356-.257l4-1.714a1 1 0 11.788 1.838L7.667 9.088l1.94.831a1 1 0 00.787 0l7-3a1 1 0 000-1.838l-7-3z"/>
                        </svg>
                    </div>
                    <div class="flex gap-1.5">
                        <div class="w-2 h-2 rounded-full bg-sky-300/40"></div>
                        <div class="w-2 h-2 rounded-full bg-cyan-300/40"></div>
                        <div class="w-2 h-2 rounded-full bg-blue-300/40"></div>
                    </div>
                </div>

                <!-- Content Area -->
                <div class="absolute inset-0 flex flex-col justify-between p-8 sm:p-10 md:p-12 lg:p-16">
                    <div></div>
                    
                    <!-- Title Section -->
                    <div class="space-y-6">
                        <!-- Glass panel untuk title -->
                        <div class="backdrop-blur-xl bg-white/10 border border-white/20 rounded-2xl p-6 sm:p-8 shadow-2xl">
                            <div class="space-y-4">
                                <div class="flex items-center gap-3">
                                    <div class="h-8 w-1 bg-gradient-to-b from-sky-300 to-cyan-400 rounded-full"></div>
                                    <span class="text-xs sm:text-sm font-medium text-sky-200 uppercase tracking-widest">Featured Info</span>
                                </div>
                                
                                <h3 class="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-light text-white leading-tight tracking-tight" 
                                    x-text="item.judul">
                                </h3>
                            </div>
                        </div>
                ...
                <div class="absolute top-0 left-0 w-20 h-20 border-l-2 border-t-2 border-sky-300/40"></div>
                <div class="absolute top-0 right-0 w-20 h-20 border-r-2 border-t-2 border-cyan-300/40"></div>
                <div class="absolute bottom-0 left-0 w-20 h-20 border-l-2 border-b-2 border-blue-300/40"></div>
                <div class="absolute bottom-0 right-0 w-20 h-20 border-r-2 border-b-2 border-sky-300/40"></div>
            </div>
        </div>
    </template>

    <!-- Refined Navigation -->
    <div class="absolute top-1/2 -translate-y-1/2 left-0 right-0 flex justify-between px-8 sm:px-12 md:px-16 lg:px-20 pointer-events-none z-20">
        <button @click="current = current === 1 ? total : current - 1" 
                class="pointer-events-auto group w-12 h-12 sm:w-14 sm:h-14 backdrop-blur-xl bg-white/10 hover:bg-white/20 border border-white/30 hover:border-white/50 rounded-xl text-white transition-all duration-300 hover:scale-105 shadow-lg flex items-center justify-center">
            <svg class="w-5 h-5 sm:w-6 sm:h-6 group-hover:-translate-x-1 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
    ...
    <!-- Minimal Counter & Dots -->
    <div class="absolute bottom-8 sm:bottom-12 left-8 sm:left-12 md:left-16 lg:left-20 right-8 sm:right-12 md:right-16 lg:right-20 flex items-center justify-between z-20">
        <!-- Dots -->
        <div class="flex gap-2">
            <template x-for="i in total">
                <button 
                    ...
                    :class="current === i ? 'w-10 sm:w-12' : 'w-8 sm:w-10'">
                    <div class="h-0.5 bg-white/20 rounded-full overflow-hidden">
                        <div class="h-full bg-gradient-to-r from-sky-300 to-cyan-300 transition-all duration-500"
        ...
        <!-- Counter -->
        <div class="backdrop-blur-xl bg-white/10 border border-white/20 rounded-full px-4 py-2 flex items-center gap-2 text-white text-sm">
            <span class="font-medium" x-text="String(current).padStart(2, '0')"></span>
            <span class="text-white/40">/</span>
            <span class="text-white/60" x-text="String(total).padStart(2, '0')"></span>
        </div>
    </div>
</section>
