...
<section class="max-w-6xl mx-auto mt-12 px-6" x-data>
  <h2 
    class="text-2xl md:text-3xl font-bold text-center mb-6 text-[#49ADFF]" 
    data-aos="fade-up"
  >
  ...
  >
    ...
    <button 
      class="hidden md:flex absolute left-0 top-1/2 -translate-y-1/2 bg-white shadow-lg p-2 rounded-full z-10 hover:bg-[#49ADFF] hover:text-white transition"
    ...
    <div 
      x-ref="carousel"
      class="flex gap-6 overflow-x-auto scrollbar-hide snap-x snap-mandatory scroll-smooth cursor-grab active:cursor-grabbing pb-4"
    >
      @foreach($ekstrakurikulers as $item)
        <div 
          class="flex-shrink-0 w-72 bg-white rounded-2xl shadow-lg overflow-hidden hover:shadow-xl transition transform hover:-translate-y-2 duration-500 snap-center"
          data-aos="zoom-in"
        >
          <div class="flex items-center justify-center bg-gray-50 h-56">
            <img 
    ...
    <button 
      class="hidden md:flex absolute right-0 top-1/2 -translate-y-1/2 bg-white shadow-lg p-2 rounded-full z-10 hover:bg-[#49ADFF] hover:text-white transition"
      @click="$refs.carousel.scrollBy({ left: 300, behavior: 'smooth' })"
    >
      &#8594;
    </button>
