        ...
        <div class="mt-12">
            <div class="flex flex-col items-center">
                <div class="flex items-center justify-center space-x-2">
                    ...
                    @else
                        <a href="{{ $pengumumanBiasa->previousPageUrl() }}" 
                           class="px-4 py-2 bg-white border-2 border-primary text-primary rounded-lg hover:bg-primary hover:text-white transition-all duration-300 font-medium shadow-md hover:shadow-lg">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"></path>
                            </svg>
                        </a>
                    ...
                    {{-- Page Numbers --}}
                    @foreach ($pengumumanBiasa->getUrlRange(1, $pengumumanBiasa->lastPage()) as $page => $url)
                        @if ($page == $pengumumanBiasa->currentPage())
                            <span class="px-4 py-2 bg-primary text-white rounded-lg font-bold shadow-lg">
                                {{ $page }}
                            </span>
                        @else
                            <a href="{{ $url }}" 
                               class="px-4 py-2 bg-white border-2 border-gray-300 text-gray-700 rounded-lg hover:border-primary hover:text-primary hover:bg-blue-50 transition-all duration-300 font-medium shadow-md hover:shadow-lg">
                                {{ $page }}
                            </a>
                        @endif
                    @endforeach
                ...
                <div class="mt-4 text-sm text-gray-600">
                    <span class="font-semibold">Menampilkan {{ $pengumumanBiasa->firstItem() }} - {{ $pengumumanBiasa->lastItem() }}</span>
                    dari
                    <span class="font-semibold">{{ $pengumumanBiasa->total() }}</span>
                    pengumuman
                </div>
            </div>
        </div>
