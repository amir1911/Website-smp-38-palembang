...
<section 
  ...
  x-data="{ scrollLeft() { $refs.carousel.scrollBy({ left: -350, behavior: 'smooth' }) }, scrollRight() { $refs.carousel.scrollBy({ left: 350, behavior: 'smooth' }) } }"
>
  <!-- Judul -->
  <h2 
  ...
  <button 
    @click="scrollLeft"
    class="hidden md:flex absolute left-0 top-1/2 -translate-y-1/2 bg-white/60 backdrop-blur-md p-3 rounded-full shadow-xl hover:bg-[#007BFF] hover:text-white transition duration-300 z-10"
  >
  ...
  <div 
    x-ref="carousel"
    class="flex gap-8 overflow-x-auto scrollbar-hide snap-x snap-mandatory scroll-smooth pb-6 cursor-grab active:cursor-grabbing px-2"
  >
    @foreach($ekstrakurikulers as $item)
      <div 
        class="flex-shrink-0 w-72 md:w-80 bg-gradient-to-br from-white via-[#f9f9ff] to-[#f3f8ff] rounded-3xl shadow-lg hover:shadow-2xl border border-white/30 transition-all duration-500 transform hover:-translate-y-3 hover:scale-[1.03] snap-center"
        data-aos="zoom-in"
      >
        <!-- Gambar -->
        <div class="relative overflow-hidden rounded-t-3xl group">
          <img 
            src="{{ asset('storage/' . $item->foto) }}" 
            alt="{{ $item->nama_kegiatan }}" 
            class="h-56 w-full object-cover group-hover:scale-110 transition-transform duration-700"
          >
          <div class="absolute inset-0 bg-gradient-to-t from-black/50 to-transparent opacity-0 group-hover:opacity-100 transition duration-500 flex items-end justify-center p-4">
  ...
  <button 
    @click="scrollRight"
    class="hidden md:flex absolute right-0 top-1/2 -translate-y-1/2 bg-white/60 backdrop-blur-md p-3 rounded-full shadow-xl hover:bg-[#007BFF] hover:text-white transition duration-300 z-10"
  >
    <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" />
    </svg>
