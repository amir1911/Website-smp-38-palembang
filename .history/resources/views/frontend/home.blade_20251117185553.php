...
<section class="relative w-screen overflow-hidden font-[Poppins] bg-gradient-to-b from-white via-blue-50/30 to-white -mx-[50vw] left-[50%] right-[50%]" 
    ...
    <template x-for="(item, index) in {{ $carousels->toJson() }}" :key="index">
        <div x-show="current === index + 1" 
             ...
             class="relative w-full h-[350px] sm:h-[450px] md:h-[550px] lg:h-[600px] xl:h-[650px]">
            ...
            <div class="relative w-full h-full overflow-hidden">
                ...
                <div class="absolute inset-0 opacity-5" style="background-image: url('data:image/svg+xml,%3Csvg width=\"60\" height=\"60\" viewBox=\"0 0 60 60\" xmlns=\"http://www.w3.org/2000/svg\"%3E%3Cg fill=\"%231e40af\" fill-opacity=\"1\"%3E%3Cpath d=\"M30 10l5 15h15l-12 9 5 15-13-9-13 9 5-15-12-9h15z\"/%3E%3C/g%3E%3C/svg%3E');"></div>
                    ...
                    <div class="flex items-center justify-between max-w-7xl mx-auto">
                        <div class="flex items-center space-x-3">
                            ...
                            <div class="hidden sm:block">
                                <div class="text-white text-xs font-medium opacity-90">Selamat Datang di</div>
            ...
            <div class="absolute inset-0 flex items-end pb-12 sm:pb-16 md:pb-20 px-6 sm:px-8 md:px-12 lg:px-16">
                <div class="w-full max-w-7xl mx-auto">
                    <div class="mb-3 sm:mb-4 animate-[slideInLeft_0.6s_ease-out]">
                        <span class="inline-flex items-center space-x-2 bg-blue-500 text-white px-4 py-1.5 sm:px-5 sm:py-2 rounded-full text-xs sm:text-sm font-semibold shadow-lg">
                    ...
                    <h2 class="text-2xl sm:text-3xl md:text-4xl lg:text-5xl xl:text-6xl font-bold text-white mb-3 sm:mb-4 leading-tight drop-shadow-[0_4px_12px_rgba(0,0,0,0.5)] animate-[slideInLeft_0.8s_ease-out_0.1s_both]" 
                        x-text="item.judul"></h2>

                    <p class="text-sm sm:text-base md:text-lg text-blue-50 max-w-2xl drop-shadow-[0_2px_8px_rgba(0,0,0,0.5)] leading-relaxed animate-[slideInLeft_1s_ease-out_0.2s_both]"
                       x-text="item.deskripsi || 'Mengembangkan potensi siswa melalui pendidikan berkualitas dan berkarakter.'"></p>

                    <div class="flex flex-wrap gap-3 sm:gap-4 mt-5 sm:mt-6 animate-[slideInLeft_1.2s_ease-out_0.3s_both]">
                        <button class="group bg-blue-600 hover:bg-blue-700 text-white font-semibold px-6 sm:px-8 py-2.5 sm:py-3 rounded-lg sm:rounded-xl transition-all duration-300 transform hover:scale-105 hover:shadow-xl flex items-center space-x-2 text-sm sm:text-base">
                        ...
                        <button class="group bg-white/20 hover:bg-white/30 backdrop-blur-sm text-white font-semibold px-5 sm:px-7 py-2.5 sm:py-3 rounded-lg sm:rounded-xl transition-all duration-300 border border-white/40 flex items-center space-x-2 text-sm sm:text-base">
    ...
    <div class="absolute inset-0 flex justify-between items-center px-4 sm:px-6 md:px-8 pointer-events-none">
        <button @click="current = current === 1 ? total : current - 1; progress = 0;" 
                class="pointer-events-auto group bg-white hover:bg-blue-600 text-blue-600 hover:text-white rounded-full p-3 sm:p-4 transition-all duration-300 transform hover:scale-110 hover:-translate-x-1 shadow-lg hover:shadow-xl">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-8">
            <div class="flex items-center justify-between">
                    ...
                    <template x-for="i in total" :key="i">
                        <button @click="current = i; progress = 0;" 
                            ...
                            <div class="h-1 sm:h-1.5 bg-blue-200 rounded-full overflow-hidden">
                                <div x-show="current === i" class="h-full bg-gradient-to-r from-blue-500 to-blue-600 rounded-full transition-all duration-100" :style="`width: ${current === i ? progress : 0}%`"></div>
                            </div>
                            <div x-show="current === i" class="absolute -top-8 left-1/2 -translate-x-1/2 bg-blue-600 text-white text-xs px-2 py-1 rounded whitespace-nowrap">
                ...
                <div class="flex items-center space-x-3 sm:space-x-4">
                    ...
                    <button @click="autoplay = !autoplay; progress = 0;" class="bg-blue-100 hover:bg-blue-200 text-blue-600 rounded-lg p-2 transition-all duration-300">
    ...
    <div class="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-8 md:gap-10 lg:gap-12 items-center px-4 sm:px-6 md:px-8">
        ...
        <div class="flex flex-col items-center" data-aos="fade-right" data-aos-duration="800">
            <div class="border-4 border-cyan-300 rounded-2xl overflow-hidden shadow-2xl w-[240px] sm:w-[280px] md:w-[320px] lg:w-[380px] xl:w-[420px] transform hover:scale-105 transition-all duration-700">
                <img src="{{ asset('storage/guru/windah.jpg') }}" alt="Kepala Sekolah" class="w-full h-[240px] sm:h-[280px] md:h-[340px] lg:h-[400px] object-cover">
            </div>
    ...
    <div class="max-w-7xl mx-auto flex flex-col-reverse lg:grid lg:grid-cols-2 gap-8 md:gap-10 lg:gap-12 items-center px-4 sm:px-6 md:px-8">
        ...
        <div data-aos="fade-right" data-aos-duration="800" class="text-center lg:text-left">
            ...
            <p class="text-gray-700 text-base sm:text-lg md:text-xl mb-6 md:mb-8 leading-relaxed">
                SMP Negeri 38 Palembang merupakan sekolah unggulan berbasis <span class="font-medium">Kurikulum Merdeka</span> yang berkomitmen membentuk generasi berkarakter, mandiri, dan kreatif dalam menghadapi tantangan masa depan.
            </p>
            <a href="/profile" class="inline-flex items-center justify-center gap-2 bg-[#49ADFF] text-white px-6 sm:px-8 py-3 sm:py-3.5 md:py-4 rounded-full shadow-md hover:bg-[#2796f0] transition-all duration-500 transform hover:scale-105 text-sm sm:text-base md:text-lg">
                Learn More
        ...
        <div class="flex justify-center mb-10 lg:mb-0" data-aos="zoom-in-up" data-aos-duration="800" data-aos-delay="200">
            <div class="bg-gradient-to-br from-[#F9FBFF] to-[#E9F3FF] shadow-xl rounded-3xl p-6 sm:p-8 md:p-10 hover:scale-105 transition-transform duration-500">
                <img src="{{ asset('storage/logo/smp38palembang.jpg') }}" alt="Logo Sekolah" class="w-[200px] sm:w-[260px] md:w-[340px] lg:w-[420px] h-auto object-contain mx-auto animate-float">
            </div>
        </div>
    </div>
</section>
...
{{-- ========================================
     EKSTRAKURIKULER
     ======================================== --}}
<section class="max-w-7xl mx-auto mt-12 sm:mt-16 md:mt-20 px-4 sm:px-6 md:px-8 relative mb-16"
         x-data="{ 
             scrollLeft() { $refs.carousel.scrollBy({ left: -350, behavior: 'smooth' }) }, 
             scrollRight() { $refs.carousel.scrollBy({ left: 350, behavior: 'smooth' }) } 
         }">
    <h2 class="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-extrabold text-center mb-8 sm:mb-10 md:mb-12 bg-gradient-to-r from-[#007BFF] to-[#00D4FF] bg-clip-text text-transparent select-none" 
        data-aos="fade-up" data-aos-duration="800">
        Kegiatan Ekstrakurikuler
    </h2>

    {{-- Navigation Buttons --}}
    <button @click="scrollLeft" class="hidden lg:flex absolute left-0 top-1/2 -translate-y-1/2 -translate-x-4 bg-white/80 backdrop-blur-md p-3 rounded-full shadow-lg hover:bg-[#007BFF] hover:text-white transition-all duration-300 z-10 hover:scale-110">
    ...
    {{-- Carousel --}}
    <div x-ref="carousel" class="flex gap-6 md:gap-8 overflow-x-auto scrollbar-hide snap-x snap-mandatory scroll-smooth pb-8 cursor-grab active:cursor-grabbing px-2">
        @foreach($ekstrakurikulers as $index => $item)
        <div class="flex-shrink-0 w-64 sm:w-72 md:w-80 lg:w-[340px] bg-white/80 backdrop-blur-xl rounded-3xl shadow-lg hover:shadow-2xl border border-white/30 transition-all duration-500 transform hover:-translate-y-2 hover:scale-[1.02] snap-center"
             data-aos="zoom-in" data-aos-duration="800" data-aos-delay="{{ $index * 100 }}">
            <div class="flex items-center justify-center h-48 sm:h-52 md:h-56 bg-gradient-to-br from-[#f7fbff] to-[#eef5ff] rounded-t-3xl">
                <img src="{{ asset('storage/' . $item->foto) }}" alt="{{ $item->nama_kegiatan }}" class="max-h-44 sm:max-h-48 md:max-h-52 max-w-[90%] object-contain rounded-xl transition-transform duration-700 hover:scale-105">
            </div>
            <div class="p-5 text-center select-none">
                <h3 class="text-base sm:text-lg md:text-xl font-semibold text-gray-800 mb-2">{{ $item->nama_kegiatan }}</h3>
                <p class="text-gray-600 text-sm md:text-base leading-relaxed">{{ Str::limit($item->deskripsi, 100) }}</p>
            </div>
        </div>
        @endforeach
    </div>

    <button @click="scrollRight" class="hidden lg:flex absolute right-0 top-1/2 -translate-y-1/2 translate-x-4 bg-white/80 backdrop-blur-md p-3 rounded-full shadow-lg hover:bg-[#007BFF] hover:text-white transition-all duration-300 z-10 hover:scale-110">
        <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" />
        </svg>
    </button>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 md:px-8 text-center">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8 lg:gap-10">
            ...
            <div data-aos="zoom-in" data-aos-duration="800" data-aos-delay="{{ $index * 150 + 300 }}"
                 class="group flex flex-col items-center text-center p-6 sm:p