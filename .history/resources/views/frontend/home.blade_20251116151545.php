...
<!-- Minimalist Carousel -->
<!-- Enhanced Minimalist School Carousel -->
<section class="relative w-full overflow-hidden font-[Poppins] bg-gradient-to-br from-sky-50 via-blue-50 to-cyan-50" 
    ...
    <template x-for="(item, index) in {{ $carousels->toJson() }}" :key="index">
        <div 
            ...
            <!-- Image dengan edge fade effect -->
            <div class="absolute inset-0 m-4 sm:m-6 md:m-8 rounded-2xl sm:rounded-3xl overflow-hidden shadow-2xl">
                <img :src="'/storage/' + item.gambar" 
                     :alt="item.judul" 
                     class="w-full h-full object-cover scale-105 transition-transform duration-700">
                
                <!-- Elegant gradient overlay dengan warna biru -->
                <div class="absolute inset-0 bg-gradient-to-br from-sky-900/70 via-blue-800/50 to-cyan-700/60"></div>
            ...
            <!-- Content -->
            <div class="absolute inset-0 flex flex-col items-start justify-end text-left text-white px-8 sm:px-14 md:px-20 lg:px-28 pb-20 sm:pb-24 md:pb-28 lg:pb-32">
                <div class="max-w-4xl space-y-4 sm:space-y-6">
                    <!-- School badge dengan icon -->
                    {{-- <div class="inline-flex items-center gap-3 group">
                        <div class="flex items-center gap-2 bg-gradient-to-r from-sky-400 to-cyan-400 px-4 py-1.5 rounded-full">
                            <svg class="w-4 h-4 text-white" fill="currentColor" viewBox="0 0 20 20">
                                <path d="M10.394 2.08a1 1 0 00-.788 0l-7 3a1 1 0 000 1.84L5.25 8.051a.999.999 0 01.356-.257l4-1.714a1 1 0 11.788 1.838L7.667 9.088l1.94.831a1 1 0 00.787 0l7-3a1 1 0 000-1.838l-7-3z"/>
                            </svg>
                            <span class="text-xs font-semibold uppercase tracking-wider">Informasi Sekolah</span>
                        </div>
                        <div class="w-20 h-0.5 bg-gradient-to-r from-sky-400/80 to-transparent"></div>
                    </div> --}}
                    
                    <!-- Title dengan improved typography -->
                    <h3 class="text-3xl sm:text-4xl md:text-5xl lg:text-6xl xl:text-7xl font-light tracking-tight leading-[1.1] text-white drop-shadow-2xl" 
                        x-text="item.judul"
                    ...
                    <div class="flex items-center gap-3">
                        <div class="w-12 h-0.5 bg-gradient-to-r from-yellow-400 to-amber-400"></div>
                        <div class="w-2 h-2 rounded-full bg-yellow-400 animate-pulse"></div>
                        <div class="w-6 h-0.5 bg-gradient-to-r from-yellow-400/60 to-transparent"></div>
                    </div>
                </div>
            </div>

            <!-- Corner decorations -->
            <div class="absolute top-8 sm:top-12 left-8 sm:left-12 w-16 h-16 sm:w-20 sm:h-20 border-l-2 border-t-2 border-sky-300/30 rounded-tl-2xl"></div>
            <div class="absolute bottom-8 sm:bottom-12 right-8 sm:right-12 w-16 h-16 sm:w-20 sm:h-20 border-r-2 border-b-2 border-cyan-300/30 rounded-br-2xl"></div>
        </div>
    </template>

    <!-- Enhanced Navigation dengan school theme -->
    <div class="absolute bottom-8 sm:bottom-10 right-8 sm:right-14 md:right-20 lg:right-28 flex items-center gap-3 sm:gap-4 z-10">
        <button @click="current = current === 1 ? total : current - 1" 
                class="group w-11 h-11 sm:w-13 sm:h-13 md:w-14 md:h-14 flex items-center justify-center border-2 border-sky-400/60 hover:border-sky-300 text-white hover:bg-sky-400/20 backdrop-blur-md bg-white/5 transition-all duration-300 hover:scale-110 rounded-lg">
            <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 sm:w-6 sm:h-6 group-hover:-translate-x-0.5 transition-transform" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                <path stroke-linecap="round" stroke-linejoin="round" d="M15 19l-7-7 7-7" />
            </svg>
        </button>
        
        <!-- Enhanced Counter -->
        <div class="backdrop-blur-md bg-gradient-to-br from-sky-400/20 to-cyan-400/20 border border-sky-300/40 rounded-lg px-4 sm:px-5 py-2.5 sm:py-3 shadow-lg">
            <div class="flex items-center gap-2 sm:gap-3 text-white">
                <span class="text-xl sm:text-2xl font-bold" x-text="String(current).padStart(2, '0')"></span>
                <div class="flex flex-col gap-0.5">
                    <div class="w-0.5 h-2 bg-sky-300/60 rounded-full"></div>
                    <div class="w-0.5 h-2 bg-cyan-300/60 rounded-full"></div>
                </div>
                <span class="text-xs sm:text-sm font-light text-white/80" x-text="String(total).padStart(2, '0')"></span>
            </div>
        </div>
    ...
    <div class="absolute bottom-8 sm:bottom-10 left-8 sm:left-14 md:left-20 lg:left-28 flex gap-2.5 sm:gap-3">
        <template x-for="i in total">
            <button 
                @click="current = i" 
                class="group relative transition-all duration-500 overflow-hidden rounded-full"
                :class="current === i ? 'w-12 sm:w-14 md:w-16' : 'w-10 sm:w-12'">
                <!-- Background bar -->
                <div class="h-1 bg-white/20 backdrop-blur-sm rounded-full overflow-hidden">
                    <!-- Active gradient indicator -->
                    <div class="h-full bg-gradient-to-r from-sky-400 via-cyan-400 to-blue-400 rounded-full transition-all duration-500 shadow-lg"
                         :class="current === i ? 'w-full' : 'w-0'">
                    </div>
                </div>
                <!-- Glow effect saat active -->
                <div class="absolute inset-0 bg-sky-400/30 blur-md rounded-full transition-opacity duration-500"
    ...
    <div class="absolute top-0 left-0 right-0 h-1 bg-sky-200/20 overflow-hidden">
        <div class="h-full bg-gradient-to-r from-sky-400 via-cyan-400 to-blue-400 transition-all duration-[5000ms] ease-linear shadow-lg shadow-sky-400/50"
             :style="'width: ' + (current === (index + 1) ? '100%' : '0%')">
        </div>
    </div>

    <!-- Floating particles untuk efek premium -->
    <div class="absolute inset-0 pointer-events-none overflow-hidden">
        <div class="absolute top-1/4 left-1/4 w-2 h-2 bg-sky-300/40 rounded-full animate-float-slow blur-sm"></div>
        <div class="absolute top-1/3 right-1/3 w-1.5 h-1.5 bg-cyan-300/30 rounded-full animate-float-slower blur-sm"></div>
        <div class="absolute bottom-1/4 left-1/3 w-2.5 h-2.5 bg-blue-300/20 rounded-full animate-float blur-sm"></div>
    </div>
</section>
