...
<!-- Section Ekstrakurikuler -->
<section 
  class="max-w-7xl mx-auto mt-16 px-6" 
...
>
  <h2 
    class="text-3xl md:text-4xl font-bold text-center mb-10 text-[#007BFF]" 
    data-aos="fade-up"
  >
  ...
  <div class="relative">
    ...
    <button 
      @click="scrollLeft"
      class="hidden md:flex absolute left-0 top-1/2 -translate-y-1/2 bg-white/80 backdrop-blur-md p-3 rounded-full shadow-lg hover:bg-[#007BFF] hover:text-white transition z-10"
    >
    ...
    <div 
      x-ref="carousel"
      class="flex gap-6 overflow-x-auto scrollbar-hide snap-x snap-mandatory scroll-smooth pb-6 px-1 cursor-grab active:cursor-grabbing"
    >
      @foreach($ekstrakurikulers as $item)
        <div 
          class="flex-shrink-0 w-72 md:w-80 bg-white rounded-3xl shadow-lg hover:shadow-2xl overflow-hidden transform transition-all duration-500 hover:-translate-y-2 snap-center"
          data-aos="zoom-in"
        >
          <!-- Gambar -->
          <div class="relative group">
            ...
            <div class="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent opacity-0 group-hover:opacity-100 transition duration-500 flex items-end justify-center p-4">
              <span class="text-white text-sm italic">{{ $item->nama_kegiatan }}</span>
            </div>
          </div>
    ...
    <button 
      @click="scrollRight"
      class="hidden md:flex absolute right-0 top-1/2 -translate-y-1/2 bg-white/80 backdrop-blur-md p-3 rounded-full shadow-lg hover:bg-[#007BFF] hover:text-white transition z-10"
    >
      <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" />
      </svg>
