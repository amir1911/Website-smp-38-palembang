...
<section class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
    ...
    <div>
        ...
        <div class="mt-12 space-y-6">
            ...
            {{-- Navigation --}}
            <div class="bg-white rounded-2xl p-5 lg:p-6 shadow-xl border border-gray-200">
                <div class="flex flex-col lg:flex-row items-center justify-between gap-6">
                    {{-- Previous Button --}}
                    <div class="flex items-center">
                        @if ($pengumumanBiasa->onFirstPage())
                            <button disabled class="flex items-center gap-2 px-4 py-2.5 bg-gray-100 text-gray-400 rounded-xl cursor-not-allowed font-semibold text-sm">
                        ...
                        @else
                            <a href="{{ $pengumumanBiasa->previousPageUrl() }}" 
                               class="flex items-center gap-2 px-4 py-2.5 bg-white hover:bg-primary text-gray-700 hover:text-white border-2 border-gray-300 hover:border-primary rounded-xl transition-all duration-300 font-semibold text-sm shadow-sm hover:shadow-md transform hover:-translate-x-1">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/>
                                </svg>
                            </a>
                    ...
                    <div class="flex items-center gap-2 flex-wrap justify-center">
                        ...
                        @if ($start > 1)
                            <a href="{{ $pengumumanBiasa->url(1) }}" 
                               class="w-10 h-10 flex items-center justify-center bg-white border-2 border-gray-300 text-gray-700 rounded-xl hover:border-primary hover:text-primary hover:bg-blue-50 transition-all font-semibold text-sm shadow-sm hover:shadow-md">
                                1
                            </a>
                            @if ($start > 2)
                        ...
                        @for ($page = $start; $page <= $end; $page++)
                            @if ($page == $pengumumanBiasa->currentPage())
                                <span class="w-10 h-10 flex items-center justify-center bg-primary text-white rounded-xl font-bold shadow-lg text-sm">
                                    {{ $page }}
                                </span>
                            @else
