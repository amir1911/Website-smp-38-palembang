...
<section 
  class="max-w-none w-full bg-gradient-to-b from-[#1D4E89] to-[#A6C9FF] py-16 sm:py-20 md:py-24 px-4 sm:px-6 md:px-8 relative"
  x-data="{ 
  ...
  <!-- Judul -->
  <h2 
    class="text-3xl sm:text-4xl md:text-5xl font-extrabold text-center text-white mb-12 select-none"
    data-aos="fade-up"
  >
  ...
  <button 
    @click="scrollLeft"
    class="hidden lg:flex absolute left-10 top-1/2 -translate-y-1/2 bg-[#1D4E89] text-white p-3 rounded-full shadow-lg hover:bg-[#163A63] transition-all duration-300 z-10 hover:scale-110"
  >
  ...
  <div 
    x-ref="carousel"
    class="flex gap-6 md:gap-8 overflow-x-auto scrollbar-hide snap-x snap-mandatory scroll-smooth pb-8 cursor-grab active:cursor-grabbing px-2 max-w-7xl mx-auto"
  >
    @foreach($ekstrakurikulers as $index => $item)
      <div 
        class="flex-shrink-0 w-64 sm:w-72 md:w-80 lg:w-[340px] bg-white rounded-3xl shadow-lg hover:shadow-2xl border border-gray-100 transition-all duration-500 transform hover:-translate-y-2 hover:scale-[1.03] snap-center"
        data-aos="zoom-in"
        data-aos-duration="800"
        data-aos-delay="{{ $index * 100 }}"
      >
        <!-- Gambar -->
        <div class="flex items-center justify-center h-48 sm:h-52 md:h-56 bg-[#F1F5FB] rounded-t-3xl">
          <img 
            src="{{ asset('storage/' . $item->foto) }}" 
            alt="{{ $item->nama_kegiatan }}" 
            class="max-h-44 sm:max-h-48 md:max-h-52 max-w-[90%] object-cover rounded-xl transition-transform duration-700 hover:scale-105"
          >
        </div>

        <!-- Konten -->
        <div class="p-5 text-center select-none">
          <h3 class="text-lg md:text-xl font-semibold text-[#1D4E89] mb-2">
            {{ $item->nama_kegiatan }}
          </h3>
  ...
  <button 
    @click="scrollRight"
    class="hidden lg:flex absolute right-10 top-1/2 -translate-y-1/2 bg-[#1D4E89] text-white p-3 rounded-full shadow-lg hover:bg-[#163A63] transition-all duration-300 z-10 hover:scale-110"
  >
    <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
<section class="py-16 sm:py-20 md:py-24 bg-gradient-to-br from-[#DCEBFA] via-[#C8DFF7] to-[#B5D5F5] relative overflow-hidden">
  
  <!-- Decorative Elements -->
  <div class="absolute inset-0 opacity-10">
    <div class="absolute top-10 left-10 w-72 h-72 bg-white rounded-full blur-3xl"></div>
    <div class="absolute bottom-10 right-10 w-96 h-96 bg-[#1D4E89] rounded-full blur-3xl"></div>
  </div>

  <div class="max-w-7xl mx-auto px-4 sm:px-6 md:px-8 text-center relative z-10">

    <!-- Judul Section -->
    <div class="mb-12 sm:mb-16" data-aos="fade-down" data-aos-duration="600">
      ...
      <div class="w-24 h-1.5 bg-white mx-auto rounded-full shadow-lg"></div>
    </div>

    <!-- Wrapper Card Container -->
    <div class="bg-gradient-to-br from-[#1D4E89] to-[#164073] rounded-3xl sm:rounded-[2rem] px-6 sm:px-10 lg:px-14 py-12 sm:py-16 shadow-2xl backdrop-blur-sm"
         data-aos="fade-up" 
         data-aos-duration="800"
         data-aos-delay="200">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 sm:gap-10">
        ...
        <article 
          ...
          class="group flex flex-col items-center text-center p-8 sm:p-10 bg-white rounded-2xl sm:rounded-3xl shadow-lg hover:shadow-2xl transform hover:-translate-y-3 transition-all duration-500 ease-out relative overflow-hidden">

          <!-- Gradient Accent on Hover -->
          <div class="absolute inset-0 bg-gradient-to-br {{ $card['color'] }} opacity-0 group-hover:opacity-5 transition-opacity duration-500 rounded-2xl sm:rounded-3xl"></div>

          <!-- Icon Container with Gradient Background -->
          <div class="relative mb-6 p-4 bg-gradient-to-br {{ $card['color'] }} rounded-2xl shadow-md group-hover:scale-110 group-hover:rotate-6 transition-all duration-500">
            <img 
              src="{{ asset('storage/sekolah/' . $card['icon']) }}" 
              alt="Icon {{ $card['judul'] }}" 
              class="w-14 h-14 sm:w-16 sm:h-16 object-contain relative z-10"
              loading="lazy"
            >
          </div>

          <!-- Judul Card -->
          <h3 class="text-[#1D4E89] font-bold text-xl sm:text-2xl mb-3 group-hover:text-[#164073] transition-colors duration-300">
            {{ $card['judul'] }}
          </h3>

          <!-- Divider Line -->
          <div class="w-16 h-1 bg-gradient-to-r {{ $card['color'] }} rounded-full mb-4 group-hover:w-24 transition-all duration-500"></div>
