...
<!-- Modern Card School Carousel -->
<section class="relative w-full overflow-hidden font-[Poppins] bg-gradient-to-br from-slate-50 via-sky-50 to-blue-50" 
    ...
    <template x-for="(item, index) in {{ $carousels->toJson() }}" :key="index">
        <div 
            ...
            class="relative w-full h-[280px] sm:h-[400px] md:h-[500px] lg:h-[600px] xl:h-[650px] flex items-center justify-center px-4 sm:px-8 md:px-12 py-12">

            <!-- Card Container -->
            <div class="relative w-full max-w-6xl h-full">
                <!-- Main Card -->
                <div class="absolute inset-0 bg-white rounded-3xl shadow-2xl overflow-hidden">
                    <!-- Split Layout -->
                    <div class="flex flex-col md:flex-row h-full">
                        <!-- Left: Content -->
                        <div class="w-full md:w-1/2 bg-gradient-to-br from-sky-400 to-blue-600 p-8 sm:p-10 md:p-12 lg:p-16 flex flex-col justify-center">
                            <div class="space-y-6">
                                <!-- School Badge -->
                                <div class="inline-flex items-center gap-2 bg-white/20 backdrop-blur-sm rounded-full px-4 py-2 border border-white/30">
                                    ...
                                    <span class="text-xs font-semibold text-white uppercase tracking-wider">Featured</span>
                                </div>
                                
                                <!-- Title -->
                                <h3 class="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-bold text-white leading-tight" 
                                    x-text="item.judul">
                                </h3>
                                
                                <!-- Divider -->
                                <div class="w-20 h-1.5 bg-yellow-300 rounded-full"></div>
                                
                                <!-- Navigation info -->
                                <div class="flex items-center gap-4 text-white/80 text-sm">
                                    <span class="font-medium">Slide <span class="text-white font-bold" x-text="current"></span></span>
                                    <span>|</span>
                        ...
                        <!-- Right: Image -->
                        <div class="relative w-full md:w-1/2 h-48 md:h-full">
                            <img :src="'/storage/' + item.gambar" 
                                 :alt="item.judul" 
                                 class="w-full h-full object-cover">
                            <div class="absolute inset-0 bg-gradient-to-l from-transparent to-sky-600/20"></div>
                            
                            <!-- Decorative corner -->
                            <div class="absolute top-6 right-6 w-16 h-16 border-t-4 border-r-4 border-white/30 rounded-tr-2xl"></div>
                        </div>
                    </div>
                </div>

                <!-- Shadow cards for depth -->
                <div class="absolute inset-0 bg-sky-200 rounded-3xl -z-10 translate-x-2 translate-y-2"></div>
                <div class="absolute inset-0 bg-sky-100 rounded-3xl -z-20 translate-x-4 translate-y-4"></div>
            </div>
        </div>
    </template>

    <!-- Elegant Navigation -->
    <div class="absolute bottom-8 sm:bottom-12 left-0 right-0 flex justify-center items-center gap-6">
        <button @click="current = current === 1 ? total : current - 1" 
                class="w-12 h-12 sm:w-14 sm:h-14 bg-white hover:bg-sky-500 text-sky-500 hover:text-white rounded-full shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-110 flex items-center justify-center group">
        ...
        <div class="flex gap-2">
            <template x-for="i in total">
                <button 
                    ...
                    :class="current === i ? 'w-12 h-3 bg-sky-500 rounded-full shadow-lg' : 'w-3 h-3 bg-gray-300 rounded-full hover:bg-sky-300'">
                </button>
            </template>
        </div>
