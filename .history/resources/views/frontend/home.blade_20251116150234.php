...
<!-- Minimalist Carousel -->
<section class="relative w-full overflow-hidden font-[Poppins] bg-gray-50" 
    ...
    <template x-for="(item, index) in {{ $carousels->toJson() }}" :key="index">
        <div 
            ...
            <!-- Minimal overlay -->
            <div class="absolute inset-0 bg-gradient-to-br from-slate-900/60 via-transparent to-slate-900/40"></div>

            <!-- Content -->
            <div class="absolute inset-0 flex flex-col items-start justify-end text-left text-white px-6 sm:px-12 md:px-16 lg:px-24 pb-16 sm:pb-20 md:pb-24">
                <div class="max-w-3xl space-y-4">
                    <div class="w-16 h-1 bg-yellow-400"></div>
                    <h3 class="text-3xl sm:text-4xl md:text-5xl lg:text-6xl xl:text-7xl font-light tracking-tight" 
                        x-text="item.judul">
                    </h3>
                </div>
            </div>
    ...
    <!-- Minimal Navigation -->
    <div class="absolute bottom-6 sm:bottom-8 right-6 sm:right-12 md:right-16 lg:right-24 flex items-center gap-4 z-10">
        <button @click="current = current === 1 ? total : current - 1" 
                class="w-12 h-12 sm:w-14 sm:h-14 flex items-center justify-center border border-white/40 hover:border-white text-white hover:bg-white/10 backdrop-blur-sm transition-all duration-300">
        ...
        <!-- Counter -->
        <div class="text-white font-light text-sm sm:text-base backdrop-blur-sm bg-white/10 px-4 py-2">
            <span x-text="String(current).padStart(2, '0')"></span>
            <span class="mx-2 text-white/50">/</span>
            <span x-text="String(total).padStart(2, '0')" class="text-white/70"></span>
        </div>
    ...
    <div class="absolute bottom-6 sm:bottom-8 left-6 sm:left-12 md:left-16 lg:left-24 flex gap-2">
        <template x-for="i in total">
            <button 
                @click="current = i" 
                class="transition-all duration-500"
                :class="current === i ? 'w-8 h-1 bg-white' : 'w-8 h-1 bg-white/30 hover:bg-white/50'">
            </button>
        </template>
    </div>
</section>
