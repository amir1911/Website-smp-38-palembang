...
<section class="relative w-full overflow-hidden font-[Poppins]" 
    ...
    <template x-for="(item, index) in {{ $carousels->toJson() }}" :key="index">
        <div 
            ...
            <div class="absolute inset-0 flex flex-col items-center justify-center text-center text-white px-4 sm:px-6 md:px-8">
                <div class="max-w-5xl">
                    <h3 class="text-2xl sm:text-3xl md:text-4xl lg:text-5xl xl:text-6xl font-bold bg-gradient-to-r from-yellow-200 via-yellow-300 to-amber-400 bg-clip-text text-transparent drop-shadow-2xl" 
                        x-text="item.judul"
    ...
    <!-- Navigation Buttons dengan design modern -->
    <div class="absolute inset-0 flex justify-between items-center px-4 sm:px-6 md:px-8 lg:px-12 pointer-events-none">
        <button @click="current = current === 1 ? total : current - 1" 
                class="pointer-events-auto group relative bg-white/10 backdrop-blur-md hover:bg-white/20 border border-white/20 text-white rounded-full p-3 sm:p-4 md:p-5 transition-all duration-300 hover:scale-110 hover:shadow-2xl hover:shadow-blue-500/50">
            <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 sm:w-6 sm:h-6 md:w-7 md:h-7 group-hover:scale-110 transition-transform" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
    ...
    <!-- Modern Indicators dengan progress bar style -->
    <div class="absolute bottom-6 sm:bottom-8 left-0 right-0 flex justify-center items-center gap-2 sm:gap-3 px-4">
        <template x-for="i in total">
            <button 
                ...
                :class="current === i ? 'w-12 sm:w-16' : 'w-2 sm:w-3'">
                <!-- Background bar -->
                <div class="h-1 sm:h-1.5 rounded-full bg-white/30 backdrop-blur-sm overflow-hidden">
                    <!-- Active indicator -->
                    <div 
                        class="h-full bg-gradient-to-r from-yellow-300 via-yellow-400 to-amber-500 rounded-full transition-all duration-500 shadow-lg shadow-yellow-500/50"
    ...
    <div class="absolute top-0 left-0 right-0 h-1 bg-white/10">
        <div class="h-full bg-gradient-to-r from-yellow-300 to-amber-500 transition-all duration-[5000ms] ease-linear"
             :style="`width: ${current === index + 1 ? '100%' : '0%'}`"
             x-init="$watch('current', value => { setTimeout(() => {}, 0) })">
        </div>
    </div>
</section>
