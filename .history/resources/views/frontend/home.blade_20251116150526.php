...
<!-- School Carousel - Biru Muda Theme -->
<section class="relative w-full overflow-hidden font-[Poppins] bg-gradient-to-br from-sky-50 to-blue-50" 
    ...
    <template x-for="(item, index) in {{ $carousels->toJson() }}" :key="index">
        <div 
            ...
            <div class="absolute inset-0">
                ...
                <!-- School Theme Overlay -->
                <div class="absolute inset-0 bg-gradient-to-br from-sky-600/85 via-blue-500/70 to-cyan-500/75"></div>
                
                <!-- Pattern Overlay untuk efek akademis -->
            ...
            <div class="absolute inset-0 flex flex-col items-center justify-center text-center text-white px-4 sm:px-6 md:px-8">
                <div class="max-w-5xl space-y-4 sm:space-y-6">
                    <!-- Badge sekolah -->
                    <div class="inline-flex items-center gap-2 bg-white/20 backdrop-blur-md border-2 border-white/40 rounded-full px-4 sm:px-6 py-2 sm:py-3">
                        ...
                        <span class="text-xs sm:text-sm font-semibold tracking-wide uppercase">Informasi Sekolah</span>
                    </div>
                    
                    <!-- Title -->
                    <h3 class="text-2xl sm:text-3xl md:text-4xl lg:text-5xl xl:text-6xl font-bold leading-tight drop-shadow-2xl" 
                        x-text="item.judul">
                    </h3>
                    
                    <!-- Decorative line -->
                    <div class="flex items-center justify-center gap-3">
                        <div class="w-12 sm:w-16 h-1 bg-white/60 rounded-full"></div>
    ...
    <!-- Navigation Buttons - School Style -->
    <div class="absolute inset-0 flex justify-between items-center px-4 sm:px-6 md:px-8 lg:px-12 pointer-events-none">
        <button @click="current = current === 1 ? total : current - 1" 
                class="pointer-events-auto group bg-white hover:bg-sky-50 text-sky-600 rounded-xl sm:rounded-2xl p-3 sm:p-4 md:p-5 transition-all duration-300 hover:scale-110 shadow-lg hover:shadow-2xl hover:shadow-sky-300/50 border-2 border-sky-200">
            <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 sm:w-6 sm:h-6 md:w-7 md:h-7 group-hover:-translate-x-1 transition-transform" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
    ...
    <!-- Indicators - School Themed -->
    <div class="absolute bottom-6 sm:bottom-8 left-0 right-0 flex justify-center items-center gap-2 sm:gap-3 px-4">
        <template x-for="i in total">
            <button 
                @click="current = i" 
                class="group relative transition-all duration-500">
                <!-- Indicator style seperti buku/halaman -->
                <div class="relative overflow-hidden rounded-lg transition-all duration-500"
                     :class="current === i ? 'w-10 sm:w-12 h-3 sm:h-4' : 'w-8 sm:w-10 h-3 sm:h-4'">
                    <div class="absolute inset-0 bg-white/40 backdrop-blur-sm"></div>
                    <div class="absolute inset-0 bg-gradient-to-r from-sky-400 to-cyan-400 transition-all duration-500"
                         :class="current === i ? 'translate-x-0' : '-translate-x-full'">
                    </div>
                    <!-- Border untuk efek buku -->
    ...
    <!-- Floating elements untuk efek playful -->
    <div class="absolute top-10 left-10 w-16 h-16 sm:w-20 sm:h-20 opacity-20 pointer-events-none">
        <svg viewBox="0 0 100 100" class="text-white">
            <circle cx="50" cy="50" r="40" fill="currentColor" class="animate-pulse"/>
        </svg>
    </div>
    <div class="absolute top-20 right-16 w-12 h-12 sm:w-16 sm:h-16 opacity-20 pointer-events-none">
        <svg viewBox="0 0 100 100" class="text-white">
            <polygon points="50,10 90,90 10,90" fill="currentColor" class="animate-pulse" style="animation-delay: 0.5s"/>
        </svg>
    </div>
    <div class="absolute bottom-32 right-10 w-14 h-14 sm:w-18 sm:h-18 opacity-20 pointer-events-none">
        <svg viewBox="0 0 100 100" class="text-white">
            <rect x="10" y="10" width="80" height="80" fill="currentColor" class="animate-pulse" style="animation-delay: 1s"/>
        </svg>
    </div>
</section>
