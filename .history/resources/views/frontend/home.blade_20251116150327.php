...
<section class="relative w-full overflow-hidden font-[Poppins]" 
    ...
    <template x-for="(item, index) in {{ $carousels->toJson() }}" :key="index">
        <div 
            ...
            <div class="absolute inset-0 bg-gradient-to-tr from-purple-900/90 via-pink-600/50 to-orange-500/70 mix-blend-multiply"></div>

            <!-- Content -->
            <div class="absolute inset-0 flex flex-col items-center justify-center text-center text-white px-4 sm:px-6 md:px-8">
                <div class="max-w-5xl space-y-6">
                    <div class="inline-block px-6 py-2 bg-yellow-400 text-black font-bold text-xs sm:text-sm uppercase tracking-widest rounded-full">
                        Featured
                    </div>
                    <h3 class="text-3xl sm:text-4xl md:text-5xl lg:text-6xl xl:text-7xl font-black uppercase tracking-tighter leading-none" 
                        style="text-shadow: 4px 4px 0px rgba(0,0,0,0.3)"
                        x-text="item.judul">
                    </h3>
    ...
    <div class="absolute inset-y-0 left-0 right-0 flex justify-between items-center px-4 sm:px-8 pointer-events-none">
        <button @click="current = current === 1 ? total : current - 1" 
                class="pointer-events-auto w-14 h-14 sm:w-16 sm:h-16 bg-yellow-400 hover:bg-yellow-500 text-black font-bold rounded-none hover:rotate-[-10deg] transition-all duration-300 shadow-2xl hover:shadow-yellow-400/50">
            <svg xmlns="http://www.w3.org/2000/svg" class="w-8 h-8 mx-auto" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="3">
    ...
    <div class="absolute bottom-8 left-0 right-0 flex justify-center gap-3">
        <template x-for="i in total">
            <button 
                ...
                :class="current === i 
                    ? 'w-12 h-12 bg-yellow-400 text-black text-xl shadow-lg shadow-yellow-400/50' 
                    : 'w-10 h-10 bg-white/30 hover:bg-white/50 text-white text-sm'"
                x-text="i">
            </button>
        </template>
    </div>
