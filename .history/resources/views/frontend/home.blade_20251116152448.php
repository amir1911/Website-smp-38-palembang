...
<!-- Ultra Minimal Lines Carousel - Fixed Image Display -->
<section class="relative w-full overflow-hidden font-['Inter'] bg-white" 
        ...
        <div 
            ...
            <div class="absolute inset-0 grid grid-cols-1 md:grid-cols-12 gap-6 md:gap-8 p-6 sm:p-8 md:p-12 lg:p-16 xl:p-20">
                ...
                <div class="md:col-span-5 lg:col-span-4 flex flex-col justify-center md:justify-end order-2 md:order-1">
                    <div class="space-y-4 sm:space-y-5 md:space-y-6">
                        ...
                        <div class="inline-flex items-center gap-2 sm:gap-3">
                            ...
                            <span class="text-[10px] sm:text-xs text-sky-600 font-semibold uppercase tracking-[0.15em] hidden sm:inline">School Info</span>
                        </div>
                        
                        <!-- Title dengan font keren -->
                        <h3 class="text-2xl sm:text-3xl md:text-4xl lg:text-5xl xl:text-6xl font-light text-slate-800 leading-[1.15] tracking-[-0.02em]" 
                        ...
                        <div class="flex items-center gap-2 sm:gap-3">
                            ...
                            <div class="w-1 h-1 bg-sky-400 rounded-full animate-pulse"></div>
                            <div class="flex-1 h-px bg-slate-200"></div>
                        </div>
                        
                        <!-- Slide Counter Info - Refined Design -->
                        <div class="inline-flex items-center gap-3 sm:gap-4 bg-slate-50 border border-slate-200 rounded-full px-4 sm:px-5 py-2.5 sm:py-3">
                            <div class="flex items-baseline gap-2">
                                <span class="text-2xl sm:text-3xl md:text-4xl font-light text-slate-800 tabular-nums tracking-tight" 
                                      style="font-family: 'Space Grotesk', monospace;"
                                      x-text="String(current).padStart(2, '0')">
                                </span>
                                <span class="text-xs sm:text-sm text-slate-400 font-medium">/</span>
                                <span class="text-sm sm:text-base text-slate-500 tabular-nums font-light" 
                                      style="font-family: 'Space Grotesk', monospace;"
                                      x-text="String(total).padStart(2, '0')">
                                </span>
                            </div>
                            <div class="w-px h-6 bg-slate-300"></div>
                            <span class="text-[10px] sm:text-xs text-slate-500 font-medium uppercase tracking-wider">Slide</span>
                        </div>
                        
                        <!-- Optional: School label for mobile -->
                        <div class="inline-flex items-center gap-2 text-xs text-slate-400 sm:hidden font-medium">
                            <div class="w-1 h-1 bg-sky-400 rounded-full"></div>
                            <span class="tracking-wide">Informasi Sekolah</span>
                        </div>
                    </div>
                </div>
                
                <!-- Right Section: Image Area - FIXED -->
                <div class="md:col-span-7 lg:col-span-8 relative min-h-[240px] sm:min-h-[280px] md:min-h-0 order-1 md:order-2">
                    ...
                    <!-- Image Container with Inner Padding - FIXED ASPECT RATIO -->
                    <div class="absolute inset-1 sm:inset-2 overflow-hidden bg-slate-50">
                        <!-- Option 1: Contain (Tampilkan full gambar, mungkin ada whitespace) -->
                        <img :src="'/storage/' + item.gambar" 
                             :alt="item.judul" 
                             class="w-full h-full object-contain transition-transform duration-700 hover:scale-105">
                        
                        <!-- Option 2: Cover tapi dengan position control (uncomment jika ingin pakai ini) -->
                        <!-- <img :src="'/storage/' + item.gambar" 
                             :alt="item.judul" 
                             class="w-full h-full object-cover object-center transition-transform duration-700 hover:scale-105"> -->
                        
                        <!-- Blue Tint Overlay -->
                        <div class="absolute inset-0 bg-gradient-to-br from-sky-500/20 via-blue-400/10 to-cyan-500/20 mix-blend-multiply pointer-events-none"></div>
                        
                        <!-- Subtle vignette -->
                        <div class="absolute inset-0 bg-gradient-to-t from-slate-900/5 to-transparent pointer-events-none"></div>
                    </div>
                    
                    <!-- Corner Marks - Animated -->
                    <div class="absolute top-0 left-0 w-3 h-3 sm:w-4 sm:h-4 border-t-2 border-l-2 border-sky-400 transition-all duration-300 hover:w-6 hover:h-6"></div>
                    <div class="absolute top-0 right-0 w-3 h-3 sm:w-4 sm:h-4 border-t-2 border-r-2 border-sky-400 transition-all duration-300 hover:w-6 hover:h-6"></div>
                    <div class="absolute bottom-0 left-0 w-3 h-3 sm:w-4 sm:h-4 border-b-2 border-l-2 border-sky-400 transition-all duration-300 hover:w-6 hover:h-6"></div>
                    <div class="absolute bottom-0 right-0 w-3 h-3 sm:w-4 sm:h-4 border-b-2 border-r-2 border-sky-400 transition-all duration-300 hover:w-6 hover:h-6"></div>
                </div>
            </div>
        </div>
    </template>

    <!-- Ultra Minimal Navigation - Refined Buttons -->
    <div class="absolute bottom-4 sm:bottom-6 md:bottom-12 lg:bottom-16 xl:bottom-20 left-4 sm:left-6 md:left-12 lg:left-16 xl:left-20 right-4 sm:right-6 md:right-auto flex items-center justify-between md:justify-start gap-3 sm:gap-4 md:gap-6 z-20">
        
        <!-- Previous Button - Refined Design -->
        <button @click="current = current === 1 ? total : current - 1" 
                class="group relative w-10 h-10 sm:w-11 sm:h-11 border border-slate-300 hover:border-sky-400 text-slate-600 hover:text-sky-600 transition-all duration-300 flex items-center justify-center flex-shrink-0 bg-white hover:bg-sky-50 shadow-sm hover:shadow-md overflow-hidden"
                aria-label="Previous slide">
            <!-- Slide effect on hover -->
            <div class="absolute inset-0 bg-gradient-to-r from-sky-50 to-cyan-50 translate-x-[-100%] group-hover:translate-x-0 transition-transform duration-300"></div>
            <svg class="w-4 h-4 relative z-10 group-hover:-translate-x-0.5 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
                <path stroke-linecap="round" stroke-linejoin="round" d="M15 19l-7-7 7-7"/>
            </svg>
        </button>
        
        <!-- Line Indicators with Numbers - Premium Design -->
        <div class="hidden sm:flex items-center gap-2 md:gap-3 flex-1 md:flex-initial">
            <template x-for="i in total">
                <button 
                    @click="current = i" 
                    class="group relative flex flex-col items-center gap-1.5 transition-all duration-500"
                    :aria-label="'Go to slide ' + i">
                    <!-- Number label -->
                    <span class="text-[10px] font-semibold tabular-nums transition-all duration-300"
                          style="font-family: 'Space Grotesk', monospace;"
                          :class="current === i ? 'text-sky-500 scale-110' : 'text-slate-300 group-hover:text-slate-500'"
                          x-text="String(i).padStart(2, '0')">
                    </span>
                    <!-- Line indicator -->
                    <div class="h-0.5 rounded-full transition-all duration-500 overflow-hidden"
                         :class="current === i ? 'w-12 md:w-16 bg-sky-400 shadow-sm' : 'w-6 md:w-8 bg-slate-300 group-hover:bg-sky-300 group-hover:w-8 md:group-hover:w-10'">
                    </div>
                </button>
            </template>
        </div>
        
        <!-- Dot Indicators with counter - Mobile only -->
        <div class="flex sm:hidden items-center gap-2 flex-1 justify-center">
            <!-- Counter box -->
            <div class="flex items-center gap-1.5 bg-slate-100 border border-slate-200 rounded px-2 py-1">
            ...
            <div class="flex gap-1">
                <template x-for="i in total">
                    <button 
                        ...
                        :class="current === i ? 'bg-sky-400 scale-125 shadow-sm' : 'bg-slate-300'"
                        :aria-label="'Go to slide ' + i">
                    </button>
                </template>
            </div>
    ...
    <div class="absolute top-0 left-0 right-0 h-0.5 bg-slate-100 z-30 overflow-hidden">
        <div class="h-full bg-gradient-to-r from-sky-400 via-cyan-400 to-blue-400 transition-all duration-[5000ms] ease-linear shadow-sm"
...
<link href="https://fonts.googleapis.com/css2?family=Plus+Jakarta+Sans:wght@300;400;500;600;700&family=Space+Grotesk:wght@300;400;500;600;700&family=Inter:wght@300;400;500;600;700&display=swap" rel="stylesheet">

<!-- Additional Tailwind config for extra small screens -->
<style>
