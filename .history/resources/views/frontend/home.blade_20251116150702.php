...
<!-- Clean Professional School Carousel -->
<section class="relative w-full overflow-hidden font-[Poppins] bg-white" 
    ...
    <template x-for="(item, index) in {{ $carousels->toJson() }}" :key="index">
        <div 
            ...
            <div class="absolute inset-0 flex flex-col md:flex-row">
                <!-- Image Side -->
                <div class="relative w-full md:w-3/5 h-full overflow-hidden">
                    <img :src="'/storage/' + item.gambar" 
                         :alt="item.judul" 
                         class="w-full h-full object-cover">
                    <div class="absolute inset-0 bg-gradient-to-r from-transparent to-sky-900/30"></div>
                </div>
                
                <!-- Content Side -->
                <div class="absolute bottom-0 left-0 right-0 md:relative md:w-2/5 bg-gradient-to-br from-sky-500 to-blue-600 flex items-center justify-center p-6 sm:p-8 md:p-12">
                    <div class="space-y-4 sm:space-y-6">
                        <!-- School Icon -->
                        <div class="w-16 h-16 sm:w-20 sm:h-20 bg-white/20 backdrop-blur-sm rounded-2xl flex items-center justify-center border-2 border-white/40">
                        ...
                        <div class="flex gap-2">
                            <div class="flex-1 h-1.5 bg-white/30 rounded-full overflow-hidden">
                                <div class="h-full bg-yellow-400 rounded-full animate-pulse" style="width: 60%"></div>
                            </div>
                        </div>
                        
                        <!-- Slide counter -->
                        <div class="flex items-center gap-3 text-white/90">
                            <span class="text-3xl font-bold" x-text="String(current).padStart(2, '0')"></span>
                            <div class="h-px flex-1 bg-white/30"></div>
                            <span class="text-sm" x-text="String(total).padStart(2, '0')"></span>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <div class="absolute top-1/2 -translate-y-1/2 left-0 right-0 flex justify-between px-4 sm:px-6 md:px-8 pointer-events-none z-10">
        <button @click="current = current === 1 ? total : current - 1" 
                class="pointer-events-auto w-12 h-12 sm:w-14 sm:h-14 bg-white hover:bg-sky-50 text-sky-600 rounded-full shadow-xl hover:shadow-2xl transition-all duration-300 hover:scale-110 flex items-center justify-center">
    ...
    <div class="absolute bottom-6 left-6 flex gap-2 z-10">
        <template x-for="i in total">
            <button 
                ...
                :class="current === i ? 'w-10 h-2 bg-sky-500' : 'w-2 h-2 bg-gray-300 hover:bg-sky-300'">
            </button>
        </template>
    </div>
</section>
