...
<section 
  ...
  x-data="{ scrollLeft() { this.$refs.carousel.scrollBy({ left: -350, behavior: 'smooth' }) }, scrollRight() { this.$refs.carousel.scrollBy({ left: 350, behavior: 'smooth' }) } }"
>
  <!-- Judul -->
  <h2 
    class="text-3xl md:text-4xl font-extrabold text-center mb-12 bg-gradient-to-r from-[#007BFF] to-[#00C4FF] bg-clip-text text-transparent select-none"
    data-aos="fade-up"
  >
  ...
  <button 
    @click="scrollLeft"
    class="hidden md:flex absolute left-0 top-1/2 -translate-y-1/2 bg-white/30 hover:bg-[#007BFF]/80 text-gray-700 hover:text-white backdrop-blur-md p-3 rounded-full shadow-xl z-10 transition duration-300"
  >
  ...
  <div 
    x-ref="carousel"
    class="flex gap-8 overflow-x-auto scrollbar-hide snap-x snap-mandatory scroll-smooth pb-6 px-2 cursor-grab active:cursor-grabbing"
  >
    @foreach($ekstrakurikulers as $item)
      <div 
        class="flex-shrink-0 w-72 md:w-80 bg-white/80 backdrop-blur-xl rounded-3xl shadow-lg hover:shadow-2xl border border-white/20 transition-all duration-500 transform hover:-translate-y-2 hover:scale-[1.02] snap-center"
        data-aos="fade-up"
      >
        <!-- Gambar -->
        <div class="relative group">
          <img 
            ...
            class="h-56 w-full object-cover rounded-t-3xl group-hover:scale-110 transition-transform duration-700"
          >
          <div class="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent opacity-0 group-hover:opacity-100 transition duration-500 flex items-end justify-center p-4">
            <span class="text-white text-sm italic tracking-wide">{{ $item->nama_kegiatan }}</span>
          </div>
        </div>
  ...
  <button 
    @click="scrollRight"
    class="hidden md:flex absolute right-0 top-1/2 -translate-y-1/2 bg-white/30 hover:bg-[#007BFF]/80 text-gray-700 hover:text-white backdrop-blur-md p-3 rounded-full shadow-xl z-10 transition duration-300"
  >
    <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" />
    </svg>
