...
<section class="max-w-7xl mx-auto px-6 py-12">
    ...
    <div class="text-center mb-16 relative">
        <div class="absolute inset-0 bg-gradient-to-r from-blue-50 to-purple-50 -skew-y-2 -z-10 rounded-3xl"></div>
        <div class="py-12">
            ...
            <div class="mt-6 w-32 h-1.5 bg-gradient-to-r from-blue-600 to-purple-600 mx-auto rounded-full shadow-lg"></div>
        </div>
    </div>
    ...
    <div class="flex flex-wrap justify-center gap-4 mb-16">
        <a href="{{ route('pengumuman.index') }}"
           ...
           {{ !isset($kategori) 
               ...
               : 'bg-white text-gray-700 border-gray-300 hover:border-blue-500 hover:text-blue-600 shadow-md hover:shadow-lg' }}">
...
@if ($pengumumanPdf->count() > 0)
    <div class="bg-gradient-to-br from-white to-blue-50 shadow-2xl border-2 border-blue-100 rounded-2xl p-8 mb-16 transform hover:scale-[1.01] transition-all duration-300">
                        ...
                        <td class="px-6 py-4 text-center">
                            <div class="flex items-center justify-center gap-3">
                                <a href="{{ route('pengumuman.show', $item->id) }}" 
                                   class="inline-flex items-center gap-1 px-4 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600 transition-all duration-200 shadow-md hover:shadow-lg transform hover:-translate-y-0.5 text-xs font-bold">
                                   <i class="fas fa-eye"></i>
                                   Detail
                                </a>
                                <a href="{{ asset('storage/' . $item->file_pdf) }}" 
                                   target="_blank" 
                                   class="inline-flex items-center gap-1 px-4 py-2 bg-green-500 text-white rounded-lg hover:bg-green-600 transition-all duration-200 shadow-md hover:shadow-lg transform hover:-translate-y-0.5 text-xs font-bold">
    ...
    <div class="grid lg:grid-cols-3 md:grid-cols-2 grid-cols-1 gap-8">
        ...
        <div class="group bg-white rounded-2xl shadow-lg hover:shadow-2xl transition-all duration-300 overflow-hidden border-2 border-gray-100 hover:border-blue-300 flex flex-col transform hover:-translate-y-2">
            ...
            <div class="p-6 flex-1 flex flex-col">
                <h3 class="font-black text-xl text-gray-800 mb-3 leading-tight group-hover:text-blue-600 transition-colors duration-300">
                    {{ $item->judul }}
                </h3>
                ...
                <a href="{{ route('pengumuman.show', $item->id) }}" 
                   class="inline-flex items-center gap-2 text-sm font-bold text-blue-600 hover:text-blue-700 group/link">
                    Baca Selengkapnya 
            ...
            <div class="px-6 py-4 bg-gradient-to-r from-gray-50 to-blue-50 text-xs text-gray-600 flex justify-between items-center border-t-2 border-gray-100">
                <span class="flex items-center gap-2 font-semibold">
                    <i class="fas fa-calendar-alt text-blue-500"></i>
                    {{ \Carbon\Carbon::parse($item->tanggal)->translatedFormat('d F Y') }}
                </span>

                <span class="bg-gradient-to-r from-blue-500 to-purple-500 text-white px-4 py-1.5 rounded-full font-bold text-xs shadow-md">
                    {{ $item->kategori?->nama_kategori ?? '-' }}
                </span>
            </div>
