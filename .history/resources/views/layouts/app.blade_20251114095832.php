...
<div x-data="{ dropProfilMobile: false }" class="space-y-2">
    <button @click="dropProfilMobile = !dropProfilMobile"
            class="w-full flex justify-between items-center font-semibold hover:bg-white/20 px-4 py-3 rounded-xl transition">
        <span>
            <i class="fas fa-user-circle mr-3 w-5"></i>PROFIL
        </span>
        <i :class="dropProfilMobile ? 'fa-chevron-up' : 'fa-chevron-down'"
           class="fas text-sm transition-transform"></i>
    </button>

    <div x-show="dropProfilMobile" x-transition
    ...
    <!-- PENGUMUMAN (TANPA DROPDOWN SESUAI PERMINTAAN SEBELUMNYA) -->
    <a href="{{ route('pengumuman.index') }}"
...
<div x-data="{ dropGuru: false }" class="space-y-2">
    <button @click="dropGuru = !dropGuru"
            class="w-full flex justify-between items-center font-semibold hover:bg-white/20 px-4 py-3 rounded-xl transition">
        <span><i class="fas fa-chalkboard-teacher mr-3 w-5"></i>TENAGA PENDIDIK</span>
        <i :class="dropGuru ? 'fa-chevron-up' : 'fa-chevron-down'"
           class="fas text-sm transition-transform"></i>
    </button>

    <div x-show="dropGuru" x-transition
         class="bg-white/10 rounded-xl overflow-hidden ml-4 shadow-inner">
        <a href="{{ route('guru.guru') }}"
           class="block px-5 py-3 hover:bg-white/20 text-sm border-b border-white/10 transition">
            <i class="fas fa-user mr-2"></i>Guru
        </a>

        <a href="{{ route('guru.staff') }}"
           class="block px-5 py-3 hover:bg-white/20 text-sm transition">
    ...
    <div x-data="{ dropKontak: false }" class="space-y-2">
        <button @click="dropKontak = !dropKontak"
                class="w-full flex justify-between items-center font-semibold hover:bg-white/20 px-4 py-3 rounded-xl transition">
            <span><i class="fas fa-envelope mr-3 w-5"></i>INFORMASI & KONTAK</span>
            <i :class="dropKontak ? 'fa-chevron-up' : 'fa-chevron-down'"
               class="fas text-sm transition-transform"></i>
        </button>

        <div x-show="dropKontak" x-transition class="bg-white/10 rounded-xl overflow-hidden ml-4 shadow-inner">
            <a href="/contact"
               class="block px-5 py-3 hover:bg-white/20 text-sm border-b border-white/10 transition">
    ...
    <form action="{{ route('search') }}" method="GET" class="pt-4 border-t border-blue-200">
        <div class="relative">
            <input type="text" name="q" placeholder="Cari..."
                   class="w-full pl-12 pr-4 py-3.5 rounded-full text-gray-700 focus:outline-none focus:ring-2 focus:ring-white shadow-lg"
                   required>
